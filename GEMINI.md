...
Query++ is an ISV-grade SQL Integrated Development Environment (IDE) tailored for Salesforce Marketing Cloud (SFMC) Architects and Campaign Managers. It focuses on reducing development time and eliminating runtime failures through a "Zen Mode" interface, intelligent autocomplete, and strict SQL guardrails.

## Project Overview

- **Core Concept:** A specialized SQL IDE for SFMC that prevents invalid commands (like `UPDATE` or `DELETE`) and provides real-time schema awareness.
...
### Applications (`apps/`)
- **`api`**: NestJS/Fastify backend handling authentication (SFMC OAuth2), metadata proxying, and session management.
...
- **Guardrails:** Real-time linting for SFMC SQL restrictions (SELECT only, no procedural elements, 6-month retention warnings for system views).
