/**
 * MCE System Data Views - Static field definitions for schema inference
 *
 * These are hardcoded MCE system data views with complete field definitions.
 * Used by the query analyzer to expand SELECT * and infer column types.
 */
...
const SYSTEM_DATA_VIEW_FIELDS = new Map<string, DataViewField[]>([
  // EMAIL DATA VIEWS
  [
    "_Sent",
    [
...
export function isSystemDataView(name: string): boolean {
  const normalizedName = name.startsWith("_") ? name : `_${name}`;
  return SYSTEM_DATA_VIEW_FIELDS.has(normalizedName);
}

export function getSystemDataViewFields(name: string): DataViewField[] {
  const normalizedName = name.startsWith("_") ? name : `_${name}`;
  return SYSTEM_DATA_VIEW_FIELDS.get(normalizedName) ?? [];
}
