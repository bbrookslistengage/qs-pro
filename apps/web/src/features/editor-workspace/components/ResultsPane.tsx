...
export function ResultsPane({ result, onPageChange, onViewInContactBuilder }: ResultsPaneProps) {
  const totalPages =
    result.pageSize > 0 ? Math.max(1, Math.ceil(result.totalRows / result.pageSize)) : 1;
  const columns = result.columns.length > 0 ? result.columns : ['Results'];
  const hasRows = result.rows.length > 0;
  const startRow = result.totalRows > 0 ? (result.currentPage - 1) * result.pageSize + 1 : 0;
  const endRow =
    result.totalRows > 0
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className={cn(
              ...
              hasRows ? "hover:text-primary-400" : "opacity-40 cursor-not-allowed"
            )}
          >
            <LinkCircle size={14} />
