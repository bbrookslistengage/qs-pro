...
describe("ShellQueryProcessor", () => {
  ...
  describe("process() - Job Dispatch", () => {
    it("routes poll-shell-query jobs to polling handler", async () => {
      ...
      expect(asyncStatusServiceStub.retrieve).toHaveBeenCalled();
    });

    it("routes execute-shell-query jobs to execution handler", async () => {
      // Arrange
      const job = createMockBullJob({}, "execute-shell-query");
      ...
      expect(runToTempFlowMock.execute).toHaveBeenCalled();
    });

    it("defaults to execution handler for jobs without poll name", async () => {
      // Arrange
      const job = createMockBullJob({}, "unknown-job-type");
  ...
  describe("handleExecute() - Job Execution", () => {
    it("decrypts sqlText before passing to flow", async () => {
      ...
      // Assert
      expect(encryptionServiceStub.decrypt).toHaveBeenCalledWith(
        "encrypted:SELECT SubscriberKey FROM _Subscribers",
      );
      expect(runToTempFlowMock.execute).toHaveBeenCalledWith(
    ...
    it("returns canceled status when run is already canceled in database", async () => {
      ...
      expect(runToTempFlowMock.execute).not.toHaveBeenCalled();
    });
  ...
  describe("Error Handling", () => {
    it("rethrows DelayedError without transformation", async () => {
      ...
      // Act & Assert
      await expect(processor.process(job as never, "token-123")).rejects.toThrow(
        DelayedError,
      );
    });
  ...
  describe("Cancellation Behavior", () => {
    ...
    it("stops polling when run is canceled mid-poll", async () => {
      ...
      expect(asyncStatusServiceStub.retrieve).not.toHaveBeenCalled();
    });
