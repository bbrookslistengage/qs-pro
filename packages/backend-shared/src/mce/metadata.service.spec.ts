...
describe('MetadataService', () => {
  ...
  describe('getFolders', () => {
    ...
    it('should paginate if MoreDataAvailable is returned', async () => {
      mockCache.get.mockResolvedValue(null);
      mockBridge.soapRequest.mockImplementation(async (_tid: string, _uid: string, _mid: string, body: string) => {
      ...
      expect(callBodies.some((b: any) => b.includes('<ContinueRequest>req-123</ContinueRequest>'))).toBe(
        true,
      );
    });
  });
  ...
  describe('getDataExtensions', () => {
    it('should fetch local and shared DEs and merge them', async () => {
      ...
      expect(result.find((r: any) => r.CustomerKey === 'DE1')).toBeDefined();
      expect(result.find((r: any) => r.CustomerKey === 'DE2')).toBeDefined();
