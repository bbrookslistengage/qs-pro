...
describe('MetadataService', () => {
  ...
  beforeEach(async () => {
    ...
    bridge = module.get<SfmcBridgeService>(SfmcBridgeService);
  ...
  describe('getFolders', () => {
    ...
    it('should fetch from SFMC if cache miss and set cache', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith('folders:t1', expect.any(Array), 600000); // 10 mins
    });
  });
    ...
    it('should fetch local and shared DEs and merge them', async () => {
      ...
      mockBridge.soapRequest
        .mockResolvedValueOnce({ // Local
          Body: { RetrieveResponseMsg: { Results: [{ CustomerKey: 'DE1', Name: 'LocalDE' }] } }
        })
        .mockResolvedValueOnce({ // Shared
          Body: { RetrieveResponseMsg: { Results: [{ CustomerKey: 'DE2', Name: 'SharedDE' }] } }
        });
  ...
  describe('getFields', () => {
    it('should cache fields by DE key', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith(`fields:t1:${deKey}`, expect.any(Array), 1800000); // 30 mins
    });
  });
});
