...
- [ ] 1.0 Complete web HTTP standardization
  - [ ] 1.1 Update 2-6 focused web tests for refactors
  ...
  - [ ] 1.2 Make `apps/web/src/services/api.ts` the single HTTP client
  ...
  - [ ] 1.3 Add feature service modules returning raw API DTOs
  ...
  - [ ] 1.4 Refactor hooks/components to call service modules only
  ...
  - [ ] 1.5 Run only the impacted web tests
...
- [ ] 2.0 Standardize shell-query module boundaries
  - [ ] 2.1 Write 2-6 focused API unit tests for the refactor
  ...
  - [ ] 2.2 Introduce a shell-query run repository interface (API-local)
  ...
  - [ ] 2.3 Implement a Drizzle-backed repository and inject via Nest token
  ...
  - [ ] 2.4 Move SSE orchestration into a dedicated provider/service
  ...
  - [ ] 2.5 Refactor controller/service to be thin and layered
  ...
  - [ ] 2.6 Run only the impacted API tests
...
- [ ] 3.0 Verify standardization and scope constraints
  - [ ] 3.1 Confirm web codebase has no production ad-hoc HTTP calls
  ...
  - [ ] 3.2 Confirm no contract changes
    - Verify shell-query endpoint shapes/status codes are unchanged (especially SSE route)
  - [ ] 3.3 Confirm no CSRF “contract” was introduced on the client
